...
public class LoginChatServiceMongodb implements IWSListener, IMsgListener {
	//grain中心的认证
	IdentityActionAbs identityActionAbs=new OriIdentityAction();
	//切换社团中心的认证
	//IdentityActionAbs identityActionAbs=new IdentityAction();
	@Override
	...
	public void LoginChatServerHandle(WsPacket wsPacket) {
		...
		UserData userData = identityActionAbs.getUser(token);
		if (userData == null) {
			// 获取用户数据为空
			return;
		}
		OnlineUserMongodb onlineUser = OnlineUserManagerMongodb.getOnlineUserByUserId(userData.getUserId());
		if (onlineUser != null) {
		...
		if (!StringUtil.stringIsNull(userData.getUserGroupTopId())) {
			friendList = identityActionAbs.getFriendList(userData.getUserGroupTopId(), token);
			// 好友列表不可能为空
		...
		// 加入在线列表
		OnlineUserMongodb newOnlineUser = new OnlineUserMongodb(userData, session, token, friendList);
		boolean result = OnlineUserManagerMongodb.addOnlineUser(newOnlineUser);
		if (!result) {
			// 这种情况不可能发生
			return;
		}
		...
		builder.setChatUser(identityActionAbs.getChatUserDataBuilder(userData, true));
		if (friendList != null) {
			for (int i = 0; i < friendList.size(); i++) {
				UserData friendUserData = friendList.get(i);
				...
				builder.addChatUserList(identityActionAbs.getChatUserDataBuilder(friendUserData, isOnline));
				// 初始化表
		...
		if (chatGroupList != null) {
			for (int i = 0; i < chatGroupList.size(); i++) {
				...
				// 初始化表
				ChatActionMongodb.createChatGroupCollection(chatGroup.getChatGroupId());
			}
		}
		...
		if (!sendResult) {
			...
			OnlineUserManagerMongodb.removeOnlineUser(newOnlineUser);
			return;
		}
		...
		chatUserOnlineBuilder.setChatUser(identityActionAbs.getChatUserDataBuilder(userData, true));
	...
	public void userOfflineHandle(MsgPacket msgPacket) {
		...
		if (!StringUtil.stringIsNull(onlineUser.getUserData().getUserGroupTopId())) {
			friendList = identityActionAbs.getFriendList(onlineUser.getUserData().getUserGroupTopId(), onlineUser.getToken());
			if (friendList != null && friendList.size() != 0) {
